...
parser.add_option("--lr", dest="lr", help="learn rate", type=float, default=1e-4)
parser.add_option("--vlen", dest="val_length", help="set the val length. def=250", default=250)
...
if options.network == 'vgg' or options.network == 'vgg16':
    ...
    C.network = 'densenet'
elif options.network == 'efficientnetb0':
    from efficientnet.keras import EfficientNetB0 as efn
    from keras_frcnn import efficient as nn
    C.network = 'efficientnet'
    model = efn()
elif options.network == 'efficientnetb1':
    from efficientnet.keras import EfficientNetB1 as efn
    from keras_frcnn import efficient as nn
    C.network = 'efficientnet'
    model = efn()
elif options.network == 'efficientnetb2':
    from efficientnet.keras import EfficientNetB2 as efn
    from keras_frcnn import efficient as nn
    C.network = 'efficientnet'
    model = efn()
elif options.network == 'efficientnetb3':
    from efficientnet.keras import EfficientNetB3 as efn
    from keras_frcnn import efficient as nn
    C.network = 'efficientnet'
    model = efn()
elif options.network == 'efficientnetb4':
    from efficientnet.keras import EfficientNetB4 as efn
    from keras_frcnn import efficient as nn
    C.network = 'efficientnet'
    model = efn()
elif options.network == 'efficientnetb5':
    from efficientnet.keras import EfficientNetB5 as efn
    from keras_frcnn import efficient as nn
    C.network = 'efficientnet'
    model = efn()
elif options.network == 'efficientnetb6':
    from efficientnet.keras import EfficientNetB6 as efn
    from keras_frcnn import efficient as nn
    C.network = 'efficientnet'
    model = efn()
elif options.network == 'efficientnetb7':
    from efficientnet.keras import EfficientNetB7 as efn
    from keras_frcnn import efficient as nn
    C.network = 'efficientnet'
    model = efn()
else:
...
if C.network != 'efficientnet':
    ...
    shared_layers = model.get_layer('top_activation').output
...
iter_num = 0
val_length = int(options.val_length)
...
record_path = os.path.join("models", options.network, options.dataset + "_record.csv")
	...
	while True:
		try:
			...
			if iter_num == epoch_length:
				...
				progbar = generic_utils.Progbar(val_length)
				val_iter_num = 0
				while(val_iter_num < val_length):
					try:
						...
						val_losses[val_iter_num, 0] = loss_rpn[1]
						val_losses[val_iter_num, 1] = loss_rpn[2]

						val_losses[val_iter_num, 2] = loss_class[1]
						val_losses[val_iter_num, 3] = loss_class[2]
						val_losses[val_iter_num, 4] = loss_class[3]
						val_iter_num += 1
						progbar.update(val_iter_num)
				...
				if curr_loss < best_loss:
					...
					keras.models.save_model(model_all, C.model_path)
